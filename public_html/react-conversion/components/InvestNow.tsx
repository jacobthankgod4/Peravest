...
const InvestNow: React.FC = () => {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      navigate('/login?redirect=/invest-now?property_id=' + propertyId);
      return;
    }
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-semibold text-gray-900">₦{calculation.principal.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-gray-600">Interest Earned:</p>
                    <p className="font-semibold text-green-600">₦{calculation.interest.toLocaleString()}</p>
                  </div>
                  <div>
